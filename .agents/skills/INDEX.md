...
| **前端标准页面布局规范** | Vue3 + Element Plus 页面布局构建指南。包含表格列表（CRUD）、场景判断、Dialog/Drawer 模式确认、`components/` 子目录规范及 `useTable`/`useForm`/`useDelete` Hooks | `layout`, `table`, `crud`, `list`, `page`, `form`, `element-plus`, `vue3`, `useTable`, `useForm`, `useDelete`, `dialog`, `drawer` | [.agents/skills/layout/SKILL.md](file:///.agents/skills/layout/SKILL.md) |
